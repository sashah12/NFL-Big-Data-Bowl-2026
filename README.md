...
Key Architectural Features:
  ...
  - Loss Function Optimization: I utilized a combination of Huber Loss and Root Mean Square Error (RMSE). Huber Loss was selected for its robustness against outliers (e.g., broken plays/deep balls), while an exponential ramp penalty was applied to sideline and endzone boundaries to enforce "stay-in-bounds" logic.
...
This model, yielding an average RMSE of 9.05 yards on all frames(including early post-snap before QB dropback), a Huber Loss of 8.61 yards for all frames and a median RMSE of 6.2 yards for peak-accuracy frames(post-dropback/pre-throwing motion), performs within a competitive margin of these industry leaders. Notably, SOTA models are typically trained on the full proprietary Next Gen Stats (NGS)
